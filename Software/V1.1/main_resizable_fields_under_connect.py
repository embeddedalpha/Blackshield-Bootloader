...
class BootloaderGUI(tk.Frame):
    def __init__(self, master):
        ...
        for col in range(10):
            df.columnconfigure(col, weight=0)
        df.columnconfigure(9, weight=1)
        ...
        self.connect_btn.grid(row=0, column=0, padx=(5, 12), pady=(5, 0), sticky="w")
        self.disconnect_btn.grid(row=0, column=1, padx=(0, 5), pady=(5, 0), sticky="w")
        ...
        info_row = ttk.Frame(df)
        info_row.grid(row=1, column=0, columnspan=10, sticky="w", padx=5, pady=(16, 6))

        for i, (label, var) in enumerate(info_fields):
            info_row.columnconfigure(i * 2 + 1, weight=0)
            ttk.Label(info_row, text=label).grid(row=0, column=i * 2, sticky="w", padx=(0, 4), pady=2)
            ttk.Entry(info_row, textvariable=var, width=14, state="readonly").grid(
                row=0, column=i * 2 + 1, sticky="w", padx=(0, 18), pady=2
            )
