...
class InfoField(ctk.CTkFrame):
    def __init__(self, master, label_text: str, variable: tk.StringVar, width: int = 118):
...
class BootloaderGUI(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.grid(row=0, column=0, sticky="nsew", padx=18, pady=18)
    ...
    def _configure_window(self, master):
        ...
        width = max(1280, int(screen_w * 0.96))
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(
            title_wrap,
            text="CustomTkinter dark dashboard for serial bootloader read / write / validate.",
            font=ctk.CTkFont(size=13),
            text_color=TEXT_MUTED,
        ).grid(row=1, column=0, sticky="w", pady=(4, 0))

    def _make_card(self, row: int, title: str, subtitle: str | None = None, *, sticky: str = "ew", min_h: int | None = None):
        card = ctk.CTkFrame(
            self,
            corner_radius=16,
            fg_color=CARD_FG,
        ...
        card.grid(row=row, column=0, sticky=sticky, pady=(0, 14))
        card.grid_columnconfigure(0, weight=1)
        if min_h is not None:
            card.grid_propagate(False)
            card.configure(height=min_h)

        head = ctk.CTkFrame(card, fg_color=CARD_HEADER, corner_radius=14)
        ...
        if subtitle:
            ...
            head.grid_rowconfigure(1, minsize=12)
    ...
    def _build_serial_card(self):
        _, body = self._make_card(
            ...
            "Choose the COM port and transport settings before opening the bootloader channel.",
        )
        body.grid_columnconfigure(1, weight=1)
        body.grid_columnconfigure(3, weight=1)
        body.grid_columnconfigure(4, weight=0)
        body.grid_columnconfigure(5, weight=0)
        body.grid_columnconfigure(6, weight=0)

        ctk.CTkLabel(body, text="COM Port", text_color=TEXT_MUTED, font=ctk.CTkFont(size=13, weight="bold")).grid(
            row=0, column=0, sticky="w", padx=(0, 8), pady=6
        )
        self.port_cb = ctk.CTkComboBox(body, values=self._list_ports() or ["No Ports"], width=220)
        self.port_cb.grid(row=0, column=1, sticky="w", padx=(0, 22), pady=6)
        if self._list_ports():
            self.port_cb.set(self._list_ports()[0])
        else:
            self.port_cb.set("Select Port")

        ctk.CTkLabel(body, text="Baud Rate", text_color=TEXT_MUTED, font=ctk.CTkFont(size=13, weight="bold")).grid(
        ...
        self.baud_cb.grid(row=0, column=3, sticky="w", padx=(0, 22), pady=6)
        self.baud_cb.set("115200")

        self.refresh_btn = ctk.CTkButton(body, text="Refresh", width=120, command=self._refresh_ports, fg_color="#334155", hover_color="#475569")
        self.refresh_btn.grid(row=0, column=4, padx=6, pady=6)

        self.open_btn = ctk.CTkButton(body, text="Open Port", width=120, command=self.open_port, fg_color=ACCENT, hover_color=ACCENT_HOVER)
        self.open_btn.grid(row=0, column=5, padx=6, pady=6)

        self.close_btn = ctk.CTkButton(body, text="Close Port", width=120, command=self.close_port, state="disabled", fg_color="#1F2937", hover_color="#374151")
        self.close_btn.grid(row=0, column=6, padx=6, pady=6)

    def _build_device_card(self):
        _, body = self._make_card(
            2,
            "Device Configuration",
            "Connect to the target and inspect the bootloader metadata returned by the MCU.",
        )
        body.grid_columnconfigure(0, weight=1)

        button_row = ctk.CTkFrame(body, fg_color="transparent")
        button_row.grid(row=0, column=0, sticky="w", pady=(0, 18))

        self.connect_btn = ctk.CTkButton(button_row, text="Connect", width=130, command=self.connect_device, state="disabled", fg_color=SUCCESS, hover_color="#15803D")
        self.connect_btn.grid(row=0, column=0, padx=(0, 12))

        self.disconnect_btn = ctk.CTkButton(button_row, text="Disconnect", width=130, command=self.disconnect_device, state="disabled", fg_color="#334155", hover_color="#475569")
        ...
        for i, field in enumerate(self.info_fields):
            field.grid(row=0, column=i, sticky="w", padx=(0, 26))

    def _build_operations_card(self):
        _, body = self._make_card(
            3,
            "Bootloader Operations",
            "Load firmware, transfer it, read it back, and validate against the MCU-reported application metadata.",
            min_h=128,
        )
        body.grid_columnconfigure((0, 1, 2, 3, 4, 5, 6), weight=1)

        self.browse_btn = ctk.CTkButton(body, text="Browse Firmware", width=160, command=self.browse_file, fg_color=ACCENT, hover_color=ACCENT_HOVER)
        self.browse_btn.grid(row=0, column=0, padx=6, pady=6, sticky="w")

        self.write_btn = ctk.CTkButton(body, text="Write Firmware", width=150, command=self.write_firmware, state="disabled", fg_color="#1F2937", hover_color="#374151")
        self.read_btn = ctk.CTkButton(body, text="Read Firmware", width=150, command=self.read_firmware, state="disabled", fg_color="#1F2937", hover_color="#374151")
        self.validate_btn = ctk.CTkButton(body, text="Validate Firmware", width=150, command=self.validate_firmware, state="disabled", fg_color="#1F2937", hover_color="#374151")
        self.erase_btn = ctk.CTkButton(body, text="Erase Firmware", width=150, command=self.erase_firmware, state="disabled", fg_color=WARNING, hover_color="#B45309")
        self.reboot_btn = ctk.CTkButton(body, text="Reboot", width=150, command=self.reboot_mcu, state="disabled", fg_color=DANGER, hover_color="#B91C1C")

        self.write_btn.grid(row=1, column=0, padx=6, pady=6, sticky="w")
        self.read_btn.grid(row=1, column=1, padx=6, pady=6, sticky="w")
        self.validate_btn.grid(row=1, column=2, padx=6, pady=6, sticky="w")
        self.erase_btn.grid(row=1, column=3, padx=6, pady=6, sticky="w")
        self.reboot_btn.grid(row=1, column=4, padx=6, pady=6, sticky="w")

        if DEBUG_MODE:
            self.next_btn = ctk.CTkButton(body, text="Next Chunk", width=150, command=self.send_next_chunk, state="disabled")
            self.abort_btn = ctk.CTkButton(body, text="Abort Write", width=150, command=self.abort_write, state="disabled", fg_color=DANGER, hover_color="#B91C1C")
            self.next_btn.grid(row=1, column=5, padx=6, pady=6, sticky="w")
            self.abort_btn.grid(row=1, column=6, padx=6, pady=6, sticky="w")

    def _build_hex_card(self):
        _, body = self._make_card(
            4,
            "Firmware Hex View",
            "Left pane shows the selected image for write. Right pane shows the firmware streamed back from the MCU.",
            sticky="nsew",
        )
        body.grid_columnconfigure(0, weight=1)
        body.grid_columnconfigure(1, weight=1)
        ...
        left = ctk.CTkFrame(body, corner_radius=14, fg_color="#101826", border_width=1, border_color=CARD_BORDER)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        left.grid_columnconfigure(0, weight=1)
        left.grid_rowconfigure(1, weight=1)
        ctk.CTkLabel(left, text="Selected Firmware (Write)", font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT_PRIMARY).grid(
        ...
        self.write_hex_box.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0, 12))

        right = ctk.CTkFrame(body, corner_radius=14, fg_color="#101826", border_width=1, border_color=CARD_BORDER)
        right.grid(row=0, column=1, sticky="nsew", padx=(8, 0))
        right.grid_columnconfigure(0, weight=1)
        right.grid_rowconfigure(1, weight=1)
        ctk.CTkLabel(right, text="MCU Firmware (Read)", font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT_PRIMARY).grid(
            row=0, column=0, sticky="w", padx=14, pady=(12, 8)
        )
        ...
        self.read_hex_box.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0, 12))
    ...
    def _build_log_card(self):
        _, body = self._make_card(
            5,
            "Activity Log",
            "Protocol traffic, status messages, validation results, and error traces are recorded here.",
            sticky="nsew",
        )
        body.grid_columnconfigure(0, weight=1)
        body.grid_rowconfigure(0, weight=1)
