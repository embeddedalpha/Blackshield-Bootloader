...
class BootloaderGUI(tk.Frame):
    def __init__(self, master):
        ...
        master.title("Blackshield Bootloader" + (" [DEBUG]" if DEBUG_MODE else ""))
        ...
        self.readback_reported_size = None  # size reported by final READ completion ACK
        self.readback_reported_crc = None   # CRC reported by final READ completion ACK
        self._offset = 0
        self._aborted = False
        ...
        device_panel = ttk.Frame(df)
        device_panel.grid(row=0, column=0, sticky="w", padx=5, pady=(5, 6))

        button_row = ttk.Frame(device_panel)
        button_row.grid(row=0, column=0, sticky="w")

        self.connect_btn = ttk.Button(button_row, text="Connect", command=self.connect_device, state="disabled")
        self.disconnect_btn = ttk.Button(button_row, text="Disconnect", command=self.disconnect_device, state="disabled")
        self.connect_btn.grid(row=0, column=0, padx=(0, 12), sticky="w")
        self.disconnect_btn.grid(row=0, column=1, sticky="w")
        ...
        info_fields = [
            ("Bootloader Version:", self.bl_ver_var),
            ("Firmware Version:", self.fw_ver_var),
            ("Product ID:", self.pid_var),
            ("Product Version:", self.pver_var),
            ("App Version:", self.appver_var),
        ]

        info_row = ttk.Frame(device_panel)
        info_row.grid(row=1, column=0, sticky="w", pady=(16, 4))

        for i, (label, var) in enumerate(info_fields):
            pair = ttk.Frame(info_row)
            pair.grid(row=0, column=i, sticky="w", padx=(0, 30))
            ttk.Label(pair, text=label).grid(row=0, column=0, sticky="w", padx=(0, 6), pady=2)
            ttk.Entry(pair, textvariable=var, width=14, state="readonly").grid(row=0, column=1, sticky="w", pady=2)

        # --- Bootloader Operations ---
        of = ttk.LabelFrame(self, text="Bootloader Operations", padding=10)
        of.grid(row=2, column=0, sticky="ew", padx=10, pady=5)
        of.columnconfigure(tuple(range(7)), weight=1)

        ttk.Button(of, text="Browse Firmware", command=self.browse_file).grid(row=0, column=0, padx=5, pady=5)
        self.write_btn = ttk.Button(of, text="Write Firmware", command=self.write_firmware, state="disabled")
        self.read_btn = ttk.Button(of, text="Read Firmware", command=self.read_firmware, state="disabled")
        self.validate_btn = ttk.Button(of, text="Validate Firmware", command=self.validate_firmware, state="disabled")
        self.erase_btn = ttk.Button(of, text="Erase Firmware", command=self.erase_firmware, state="disabled")
        ...
        self.erase_btn.grid(row=1, column=3, padx=5)
        self.reboot_btn.grid(row=1, column=4, padx=5)
        ...
        if DEBUG_MODE:
            ...
            self.next_btn.grid(row=1, column=5, padx=5)
            self.abort_btn.grid(row=1, column=6, padx=5)
    ...
    def _update_validate_button_state(self):
        ...
        self.validate_btn.config(state="normal" if validate_ready else "disabled")
    ...
    def read_firmware(self):
        ...
        while True:
            ...
            if saw_end_of_data and resp["length"] >= 8:
                app_size, app_crc = self._try_parse_read_complete_ack(resp)
                if app_size is not None:
                    self.readback_reported_size = app_size
                    self.readback_reported_crc = app_crc
                    self._log(
                        f"READ completion ACK: size={app_size} bytes, CRC=0x{app_crc:08X}"
                    )
                    break
    ...
    def validate_firmware(self):
        if not self.firmware_data:
            messagebox.showwarning("Validate FW", "Load the firmware file first.")
            return

        if not self.readback_data:
            messagebox.showwarning("Validate FW", "Read the firmware back from the MCU first.")
            return

        if self.readback_reported_size is None or self.readback_reported_crc is None:
            messagebox.showwarning("Validate FW", "No final READ completion ACK with size/CRC has been captured yet.")
            return
