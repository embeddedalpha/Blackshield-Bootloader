...
class BootloaderGUI(tk.Frame):
    def __init__(self, master):
        ...
        for col in range(10):
            df.columnconfigure(col, weight=1)
        ...
        self.connect_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.disconnect_btn.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        ...
        for i, (label, var) in enumerate(info_fields):
            ...
            ttk.Label(df, text=label).grid(row=1, column=label_col, sticky="e", padx=(5, 2), pady=2)
            ttk.Entry(df, textvariable=var, width=14, state="readonly").grid(
                row=1, column=entry_col, sticky="ew", padx=(2, 8), pady=2
            )
